...
key_detected = False  # Tracks key press in the current cycle
key_detected_cycles = 0  # Tracks cycles where at least one key was detected
detection_window_active = False  # Flag for active detection window
kõik = 0

# Function to handle key press
def on_key_press(key):
    global key_detected, detection_window_active
    # Detect key press only during the active detection window
    if detection_window_active and not key_detected:
        key_detected = True  # Set flag when a key is pressed
...
# Calculate elapsed time
    ...
    # Activate detection window after 10 seconds
    if not detection_window_active and elapsed_time >= datetime.timedelta(seconds=3):
        detection_window_active = True
        window_end_time = datetime.datetime.now() + datetime.timedelta(seconds=3)
        print("Detection window active. Press any key during this cycle.")

    # Check for key press during the 10-second window
    if detection_window_active:
        if key_detected:
            key_detected_cycles += 1  # Increment the cycle count
            print("Key detected during this cycle.")
            kõik += 1
            key_detected = False  # Reset key detection flag
            detection_window_active = False  # End the detection window for this cycle
            start_time = datetime.datetime.now()  # Reset start time for the next cycle
        elif datetime.datetime.now() >= window_end_time:
            print("Detection window ended without key press.")
            kõik += 1
            detection_window_active = False  # End the detection window
            start_time = datetime.datetime.now()  # Reset start time for the next cycle    
##########################################
